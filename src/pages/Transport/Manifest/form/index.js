...
class AddMenifestListForm extends Component {
	        ...
	        if (data && data != null) {
              ...
              for (var i = baglistscanneddata.length; i > 0; i--) {
                baglistsorted.push(baglistscanneddata[i-1]);
		        	...
		        	return this.setState({rowdata: data, req_bagnumber: '', loaderflag: false,});
	        	// }else{
  ...
  updateEntity = () => {
    ...
    baglistscanneddata.map((baglistdata, index) => (      
  ...
  saveEntity = () => {
    ...
    if (this.state.menifesttype == 23) {
      postEntity('manifest', {
        ...
        destination_name: 'Destination Id- '+this.state.node_id,
        destination_id: this.state.node_id,
        vehicle_id: this.state.add_plate_number,
        //police_no: this.state.platenumname,
        driver_id: this.state.driver_id,
        //driver_name: this.state.drivername,
        checker_id: this.state.checker_id,
    ...
    if (baglistgetitem.length > 0) {
      ...
      for (let i = 0; i < baglistgetitem.length; i++) {
        if (baglistgetitem[i].total_weight) {
          bag_connoteSum += parseInt(baglistgetitem[i].total_weight);
        }
      }
    }

    if (baglistsorted.length > 0) {
      totalbag += baglistsorted.length;
      for (let i = 0; i < baglistsorted.length; i++) {
        if (baglistsorted[i].weight) {
          bag_connoteSum += parseInt(baglistsorted[i].weight);
        }
      }
    }
                ...
                <div>
                  ...
                  <div className={classes.tableWrapper}>
                    ...
                    <Table className={classes.table}>
                      ...
                      <TableBody>
                        {baglistsorted.map((scanneddata, index) => (
                              <TableRow
                                hover
                                tabIndex={-1}
                                key={index}
                              >
                                <TableCell padding="none">{scanneddata.bag_id ? scanneddata.bag_id : '-'}</TableCell>
                                <TableCell>{scanneddata.bag_no ? scanneddata.bag_no : '-'}</TableCell>
                                <TableCell>{scanneddata.weight ? scanneddata.weight : 0}</TableCell>
                                <TableCell>{scanneddata.connote_qty ? scanneddata.connote_qty : 0}</TableCell>
                                <TableCell>{scanneddata.destination ? scanneddata.destination : '-'}</TableCell>
                              </TableRow>
                         ))
                        }
